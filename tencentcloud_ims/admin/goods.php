...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    // 图片内容安全检查
    if (function_exists('is_ims_enable') && is_ims_enable())
    {
        $ims_images = array_unique($ims_images);
        include_once '../includes/cls_tencentcloud_ims.php';
        $ims_options = get_ims_options();
        $tencent_ims = new tencent_ims($ims_options);
        if (!empty($ims_images) && !$tencent_ims->AuditImages(ROOT_PATH, $ims_images)) {
            sys_msg($_LANG['tencentcloud_image_audit_no'], 1, array(), false);
        }
    }
...
elseif ($_REQUEST['act'] == 'ajax_upload_image')
{
    ...
    if (function_exists('is_ims_enable') && is_ims_enable())
    {
        $ims_images = array_unique(array($source_img));
        include_once '../includes/cls_tencentcloud_ims.php';
        $ims_options = get_ims_options();
        $tencent_ims = new tencent_ims($ims_options);
        if (!empty($ims_images) && !$tencent_ims->AuditImages(ROOT_PATH, $ims_images)) {
            $img_info = array('img_id'=>$_POST['id'], 'msg'=>$_LANG['tencentcloud_image_audit_no']);
            make_json_error($img_info);
        }
    }
